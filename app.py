...
def process_image(img, strength):
    ...
    hsv[:, :, 0] = np.clip(hsv[:, :, 0] + np.random.randint(-HSV_H_SHIFT, HSV_H_SHIFT + 1, (h, w)), 0, 179)
    hsv[:, :, 1] = np.clip(hsv[:, :, 1] + np.random.randint(-HSV_S_SHIFT, HSV_S_SHIFT + 1, (h, w)), 0, 255)
    hsv[:, :, 2] = np.clip(hsv[:, :, 2] + np.random.randint(-HSV_V_SHIFT, HSV_V_SHIFT + 1, (h, w)), 0, 255)
...
output_format = st.radio("出力形式", ["jpg", "gif"], horizontal=True)
...
if uploaded:
    ...
    st.image(result, caption=f"加工済み画像（強度 {strength}）")
